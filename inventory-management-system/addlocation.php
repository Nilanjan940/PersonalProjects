...
if (isset($_POST['addlocation'])) {
    ...
    $checkQuery = "SELECT * FROM location WHERE name = '$locationName'";
    ...
    if (mysqli_num_rows($checkResult) > 0) {
        ...
        if ($insertResult) {
            echo "<script>alert('Location added successfully!'); window.location.href='viewlocation.php';</script>";



        } else {
            $error = "Error occurred while adding location. Please try again.";
        }
    }
}
...
<li class="navList">
    ...
    <p class="loginform-title">Add Location</p>
    <?php if ($error) { ?>
        <p class="error"><?php echo $error; ?></p>
    <?php } ?>
    <div class="input-container">
        <input required="" type="text" placeholder="Enter location name" class="form-control" name="locationName">
    </div>

    <div class="input-container">
        <textarea name="address" rows="4" required placeholder="Enter address" class="form-control"></textarea>
    </div>
    <button class="submit" name="addlocation">Add location</button>
</form>
</div>
            </div>
        </div>
